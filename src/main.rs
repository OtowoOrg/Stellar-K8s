...
async fn main() -> Result<(), Error> {
    ...
    match args.command {
        ...
        Commands::Info(info_args) => {
            ...
            let api: kube::Api<StellarNode> = kube::Api::all(client);
            let list = api.list(&Default::default()).await.map_err(Error::KubeError)?;
            println!("Managed Node count: {}", list.items.len());
            return Ok(());
        }
        Commands::Run(run_args) => {
            ...
            let namespace = run_args.namespace.clone();

            let mtls_config = if run_args.enable_mtls {
                ...
                None
                    ...
                    let state = Arc::new(controller::ControllerState {
                        client: client.clone(),
                        enable_mtls: run_args.enable_mtls,
                        operator_namespace: run_args.namespace.clone(),
